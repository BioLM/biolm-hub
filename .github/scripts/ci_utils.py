...
# git's well-known empty-tree object. Used as the diff base when ``base_ref``
# has no parent (e.g. HEAD^ on a repo's initial commit), so an initial commit
# reports every tracked file as changed instead of aborting on a missing revision.
_EMPTY_TREE = "4b825dc642cb6eb9a060e54bf8d69288fbee4904"


def _ref_resolves(ref: str) -> bool:
    """Return True if ``ref`` resolves to a commit in the current repo."""
    return (
        subprocess.run(
...
def get_changed_files(base_ref: str = "HEAD") -> list[str]:
    ...
    If ``base_ref`` cannot be resolved (e.g. ``HEAD^`` on a repo's initial
    commit, which has no parent), fall back to git's empty-tree object so the
    diff lists every tracked file rather than aborting on a missing revision.
    """
