...
class TestIsNonCode:
    ...
    def test_all_non_code_extensions_covered(self):
...
class TestListSubdirectories:
    ...
    def test_models_dir_returns_models(self):
    ...
    def test_exclude_none_defaults_empty(self):
...
class TestWriteGithubOutputs:
    ...
    def test_prints_to_stdout_without_env(self, capsys, monkeypatch):
    ...
    def test_empty_outputs_dict(self, tmp_path, monkeypatch):
        output_file = tmp_path / "github_output"
        output_file.touch()
        monkeypatch.setenv("GITHUB_OUTPUT", str(output_file))
...
class TestConstants:
    ...
    def test_excluded_model_dirs(self):
        assert EXCLUDED_MODEL_DIRS == {"commons", "scripts", "__pycache__"}

    def test_non_code_extensions(self):
