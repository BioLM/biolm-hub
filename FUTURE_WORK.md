...
This is the public, openly-scoped roadmap. We keep deferred work *recorded* so outside
...
runtime that the baked image actually contains the weights. An incomplete or stale build-cache layer can
therefore crash-loop silently (e.g. a cached image missing a required config file). Proposed: a
commons-level runtime `required_files` check at container start that falls back to
`download_model_assets()` when the bake is incomplete, turning a silent crash-loop into a
slower-but-successful cold start. Deferred: it's a commons runtime-path change touching every model's
startup, and fresh deploys bake correctly.
