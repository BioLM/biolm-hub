...
def load_model_modified(checkpoint_path: str = ""):
    """Load raw/FT IF1 model"""
...
def get_pdbs_logits(
    ...
    print("Using modified antiscript")
...
def sample_new_sequences_CDR_HL(
    ...
    if light_chain:
        df_H, df_L = get_dfs_HL(df, light_chain=light_chain)
    else:

        df_H = get_dfs_HL(df, light_chain=light_chain)
...
def get_dfs_HL(df, light_chain=None):
    ...
    if light_chain:
        ...
        Hchain = df["pdb_chain"].unique()[:1]  # Assume heavy
        return df.loc[df["pdb_chain"] == Hchain[0], :]
...
def get_df_seqs_HL(df, light_chain=None):
...
def sample_from_df_logits_modified(
    df_logits,
    sample_n=1,
    sampling_temp=0.20,
    regions_to_mutate=["CDR1", "CDR2", "CDR3"],
    ...
    if light_chain:
        # Get original H/L sequence
        H_orig, L_orig = get_df_seqs_HL(df_logits, light_chain=light_chain)
        ...
        # Only sampling from heavy, light chains
        df_logits_HL = df_logits.iloc[: len(H_orig), :]
        seq = "".join(H_orig)
    ...
    for t in sampling_temp:
        ...
        for n in range(sample_n):
            ...
            if light_chain:
                ...
                # Get mutated H/L sequence
                H_mut, df_mut = sample_new_sequences_CDR_HL(
                    df_logits_HL,  # DataFrame with residue probabilities
                    t=t,  # Sampling temperature
                    imgt_regions=regions_to_mutate,  # Region to sample
                    exclude_heavy=exclude_heavy,  # Allow mutations in heavy chain
                    ...
                    # Only mutate as many positions are expected from temperature
                    verbose=verbose,
                    light_chain=light_chain,
                )
