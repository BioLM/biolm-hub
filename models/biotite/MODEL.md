...
1. **Chain extraction** (`generate` action): Parse PDB structures and extract individual chains with their sequences and atomic coordinates. The `generate` verb follows the catalog's action-verb convention; this is a **utility extraction operation**, not ML-based generation.
2. **RMSD computation** (`predict` action): Compute root-mean-square deviation between two structures after optimal superimposition. The `predict` verb follows the catalog's action-verb convention; this is a **structural metric computation**, not ML-based property prediction.
