...
def setup_workflow_source_layer(
    workflow_slug: str,
    include_models: bool = True,
) -> Callable[[modal.Image], modal.Image]:
    """
    Adds common dependencies and workflow-specific files to a Modal image.
    Also optionally includes the models directory for workflows that need it.

    Args:
        workflow_slug: The workflow's slug (e.g., "xgboost", "abworkflow")
        include_models: Whether to include the models directory (default: True)
    ...
    Example:
        ```python
        from models.commons.modal.source import setup_workflow_source_layer

        image = modal.Image.debian_slim(python_version="3.12")
        image = setup_workflow_source_layer("xgboost", include_models=True)(image)
        ```
    """
    # Convert slug to folder name (hyphens to underscores)
    workflow_folder_name = workflow_slug.replace("-", "_")
    ...
    def add_files(image: modal.Image) -> modal.Image:
        ...
        # Use relative paths for Modal's add_local_dir
        local_workflows_base = Path("workflows")
        remote_workflows_base = Path("/root/workflows")

        # Define workflow paths
        local_workflow_path = local_workflows_base / workflow_folder_name
        remote_workflow_path = remote_workflows_base / workflow_folder_name
        # Add workflow directory
        image = image.add_local_dir(
            local_workflow_path,
            str(remote_workflow_path),
            ignore=_PYCACHE_IGNORE,
            copy=True,
        )

        # Optionally add models directory if needed
        if include_models:
            local_models_base = Path("models")
            remote_models_base = Path(remote_models_path)

            # Add models commons and init
            local_commons_path = local_models_base / "commons"
            local_init_path = local_models_base / "__init__.py"
