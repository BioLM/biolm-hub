...
def _build_fixture_generation_suite() -> TestSuite:
    ...
    R2 reads live here, NOT at module scope, so importing this module (e.g. for
    `pytest --collect-only` via test.py) never touches R2. Only `generate()`
    (run explicitly) loads inputs from R2.
    """
    base_path = f"{r2_test_data_dir}/models/{ESM2Params.base_model_slug}"
    single_seq_data = read_json_from_r2(
        r2_bucket_name, f"{base_path}/{SINGLE_SEQ_INPUT}"
    )
