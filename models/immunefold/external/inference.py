...
from carbonmatrix.data.dataset import (
    AbStructureDataNpzIO,
    SeqDatasetDirIO,
    SeqDatasetFastaIO,
)
...
def save_batch_pdb(values, batch, pdb_dir, step=None, mode=None):
    N = len(batch["str_seq"])
    pred_atom14_coords = to_numpy(
        values["heads"]["structure_module"]["final_atom14_positions"]
    )
    names = batch["name"]
    ...
    for i in range(N):
        ...
        save_pdb(
            multimer_str_seq,
            pred_atom14_coords[i, : len(str_seq)],
            pdb_file,
            chain_ids,
            single_plddt,
        )
...
def immunefold(model, batch, cfg):
    with torch.no_grad():
        ...
        ptm = _compute_ptm(
            ret, batch["mask"], chain_id=batch["chain_id"], interface=True, batch=batch
        )
...
def immunefold_with_confidence(model, batch, cfg):
    with torch.no_grad():
        ret = model(batch, compute_loss=True)
        ptm = _compute_ptm(
            ret, batch["mask"], chain_id=batch["chain_id"], interface=True, batch=batch
    ...
    save_batch_pdb(ret, batch, cfg.output_dir)
    return str_ptm, plddt.to("cpu").tolist(), str_plddt
...
def predict(cfg):
    if cfg.data_io == "dir":
        ...
        dataset = AbStructureDataNpzIO(
            cfg.fasta, cfg.ag, cfg.get("contact_idx", None), cfg.type
        )
    ...
    for batch in test_loader:
        ...
        logging.info(
            "str_len= {}".format(",".join([str(len(x)) for x in batch["str_seq"]]))
        )
...
def predict_with_model(cfg, model):
    ...
    for batch in test_loader:
        ...
        ptm, plddt, full_plddt = immunefold_with_confidence(model, batch, cfg)
        results.append(
            {"ptm": float(ptm), "plddt": plddt, "full_plddt": float(full_plddt)}
        )
    return results
