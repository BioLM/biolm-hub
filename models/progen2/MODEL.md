...
Parameter counts and layer/head configurations are confirmed from paper Table 1 (Nijkamp et al., 2023). The paper also describes PROGEN2-small (151M, same architecture as OAS), PROGEN2-base (764M, same as medium but with 2048 context), and PROGEN2-xlarge (6.4B, 32 layers, 16 heads, head dim 256) which are not deployed on BioLM. The BioLM OAS variant uses a 2048 context length (matching PROGEN2-base) rather than the paper's 1024 for PROGEN2-small.
